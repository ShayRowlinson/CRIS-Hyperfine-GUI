...
class Binning(ttk.Frame):
    """
    Dopplershift and bins CRIS scan data.
    ...
    def bin_current_scan(self):
        """
        Bin only the scan currently selected in the file menu dialogue.
        """
        try:
            scan_folder = self.folder_var.get()
            if not scan_folder:
    ...
    def plot_results(self, data, D):
        """
        Plot either the hyperfine spectra or the full 3D histogram layout
        """
    ...
    def _redraw_plot_mode(self):
        """
        Redraw the plot if the plot mode has changed.
        """
