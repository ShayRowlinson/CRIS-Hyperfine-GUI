...
And with machine learning parameter estimation
...
class Fitting(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.Jl_var = DoubleVar(value=DEFAULT_J_L)
        self.Ju_var = DoubleVar(value=DEFAULT_J_U)
    ...
    def _build_gui(self):
        ...
        ttk.Radiobutton(control_frame, text="Fit 2", variable=self.fit_select_var, value=2, command=lambda: self._on_fit_toggle(2)).grid(row=0, column=2, padx=(0,10))
        # Active checkboxes
        ttk.Checkbutton(control_frame, text="Active", variable=self.fit1_active, command=lambda: self._on_active_toggle(1)).grid(row=0, column=1, padx=(0,12))
        ...
        ttk.Button(button_frame, text="Estimate Parameters", command=self._estimate_parameters).grid(row=0, column=5, padx=5)
        ttk.Button(button_frame, text="Clear Fit", command=self._clear_selected_fit).grid(row=0, column=6, padx=5)
    ...
    def _import_scan(self):
        ...
        for sep in [";", ","]:
            try:
                temp = pd.read_csv(self.filepath, sep=sep)
                if all(col in temp.columns for col in ["x", "y", "yerr", "bunches"]):
                    df = temp
                    break
            except Exception:
                continue
        ...
        if df is None:
            messagebox.showerror("CSV Error", "Could not read CSV file with required columns ('x', 'y', 'yerr', 'bunches').\nCheck delimiter and headers.")
            self.data = None
            return
    ...
    def _import_parameters(self):
        ...
        try:
            ...
            self.A_l_vars[fit].set(get_value('A_l'))
            ...
            if missing_params:
                msg += "\nMissing (set to 0): " + ", ".join(missing_params)
            messagebox.showinfo("Parameter Import Summary", msg)
    ...
    def _estimate_parameters(self):
        ...
        model_path = os.path.join(os.path.dirname(__file__), "ml_models", "fit_predictor.pkl")
        if not os.path.exists(model_path):
            messagebox.showerror("Error", f"ML model not found at {model_path}")
            return
        ...
        try:
            ...
            element_to_idx = model_data.get("element_to_idx", {e: i for i, e in enumerate(elements)})
            x_grid = np.array(model_data["x_grid"], dtype=float)
    
            if element not in element_to_idx:
                messagebox.showerror("Error", f"Element '{element}' not in trained ML model.")
                return
    
            # Prepare features
            x_vals = self.data["x"].to_numpy()
            y_vals = (self.data["y"] / self.data["bunches"]).to_numpy()
    
            # Normalise spectrum by area under curve
            y_area = np.trapezoid(np.abs(y_vals), x_vals) if y_vals.size else 1.0
            if not np.isfinite(y_area) or y_area <= 0:
                y_area = 1.0
            y_norm = y_vals / y_area
    
            # Interpolate to model x_grid
            y_interp = np.interp(x_grid, x_vals, y_norm)
    
            element_vec = [0] * len(elements)
            element_vec[element_to_idx[element]] = 1
    
            try:
                mass_int = int(mass_number)
            except ValueError:
                mass_int = hash(mass_number) % 10000
    
            features = [mass_int] + element_vec + list(y_interp)
    
            # Predict
            pred = model.predict([features])[0]  # [A_l, A_u, B_l, B_u, centroid]
    
            fit = self.selected_fit
            self.A_l_vars[fit].set(pred[0])
            self.A_u_vars[fit].set(pred[1])
            self.B_l_vars[fit].set(pred[2])
            self.B_u_vars[fit].set(pred[3])
            self.df_vars[fit].set(pred[4])
    
            messagebox.showinfo("ML Estimate Applied",
                                f"Estimated parameters applied for {element}-{mass_number} (fit {fit}).")
    
        except Exception as e:
            messagebox.showerror("ML Error", f"Failed to estimate parameters: {e}")